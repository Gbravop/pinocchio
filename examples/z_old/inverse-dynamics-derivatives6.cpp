...
int main(int argc, char ** argv)
{
    ...
    SMOOTH(n_runs){
        ...
        double time_FO_FD = dt_loop.count()*1e6;
        T_FO[_smooth] = time_FO_FD;
        // Get the running time
        // double time_SO_FD = timer.toc();
        // std::cout << "Run Time ABA SO derivatives = " << time_SO_FD << std::endl;
    }
    double time_FO_FDT = timer.toc()/n_runs;
    ...
    std::cout << "Time SO ABA_0: " << T_FO[0] << std::endl;
    std::cout << "Time SO ABA_n: " << T_FO[n_runs-1] << std::endl;
    std::cout << "Time SO ABA_T: " << time_FO_FDT << std::endl;

    // Opening file to save data:
    std::ofstream outfile;
    std::string filewrite = "time_data_FO_PINOCCHIO.csv";
    outfile.open(filewrite);

    outfile << "T_FO" << std::endl;

    for(int ii = 0; ii < (n_runs); ii++){
        outfile << T_FO[ii] << std::endl;
    }
