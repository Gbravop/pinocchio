...
//                 : argv[1];
  ...
  for (int i=0; i< (n_runs); i++){
    ...
    Minv_neg = -Minv;
    // 
    ComputeRNEASecondOrderDerivatives(model, data, q, v, a, dtau2_dqdq, dtau2_dvdv, dtau2_dqdv, dtau2_dadq);
    //
  ...
  std::cout << "Time SO ABA: " << T_SO[0] << std::endl;
  std::cout << "Time SO ABA: " << T_SO[n_runs-1] << std::endl;
}
